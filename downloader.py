        ...
        for model in models:
          if not self.skip_failing_scenarios or (experiment not in unavailable_experiments or not (model in unavailable_experiments[experiment])):
            try:
              ...
              filename = f'{DATADIR}cmip6_{frequency}_{start}-{end}_{experiment}_{model}.{fileformat}'
...
c = cdsapi.Client() # Doc: https://cds.climate.copernicus.eu/toolbox/doc/how-to/1_how_to_retrieve_data/1_how_to_retrieve_data.html
...
def download(models, experiments, DATADIR, variable='near_surface_air_temperature', frequency='monthly', area=None, mark_failing_scenarios=False, skip_failing_scenarios=False, forecast_from=2015, start=1850, end=2100, fileformat='zip'): 
  ...
  unavailable_experiments = status['unavailable_experiments'][variable] if skip_failing_scenarios else {}
